...
import { BookOpen, Clock, Star, Users, ChevronRight, Pen, Award, Search, Filter, RotateCcw } from 'lucide-react';
...
const GrammarList = () => {
    ...
    useEffect(() => {
        const fetchLessons = async () => {
            ...
            try {
                ...
                const response = await getAllLessons('grammar', pagination.limit, pagination.offset);
                console.log('Grammar lessons response:', response);
                if (response.status === 'success') {
                    const grammarLessons = response.data.rows || response.data || [];

                    // Lấy thông tin tiến độ cho từng bài học
                    const lessonsWithProgress = await Promise.all(
                        grammarLessons.map(async (lesson) => {
                            try {
                                const progressResponse = await getLessonProgressAndAnswers(lesson.lesson_id, 'grammar');
                                let progressData = null;
                                let completedQuestions = 0;
                                let totalQuestions = lesson.exercise_count || lesson.quiz_questions?.length || 5;
                                ...
                                return {
                                    ...
                                    //content_preview: lesson.content?.substring(0, 150) + '...' || 'No preview',
                                    ...
                                    category: lesson.category || 'General Grammar',
                        ...
                        level: lesson.level || 'beginner',
                        duration: '15-20 phút',
                        total_questions: lesson.exercise_count || lesson.quiz_questions?.length || 5,
                    ...
                    setPagination(prev => ({ ...prev, total: response.pagination?.total || grammarLessons.length }));
                } else {
    ...
    const handleLessonAction = (lesson, action) => {
        if (action === 'review') {
            ...
            navigate(`/grammar/${lesson.lesson_id}?mode=review`);
        } else if (action === 'retry') {
            // Làm lại bài học
            navigate(`/grammar/${lesson.lesson_id}?mode=retry`);
        } else {
            // Vào bài học bình thường (start hoặc continue)
        navigate(`/grammar/${lesson.lesson_id}`);
        }
    };
            ...
            <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white">
                <div className="max-w-7xl mx-auto px-4 py-16">
                    <div className="text-center">
                        ...
                        <h1 className="text-4xl font-bold mb-4">Grammar Lessons</h1>
                        <p className="text-xl text-blue-100 max-w-2xl mx-auto">
                            Thành thạo ngữ pháp tiếng Anh với các bài học toàn diện, ví dụ minh họa và bài trắc nghiệm tương tác.
                        </p>
                    </div>
                </div>
            </div>
                    ...
                    {filteredLessons.map((lesson) => {
                        ...
                        <div key={lesson.lesson_id} className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
                            <div className="p-6">
                                ...
                                <p className="text-sm text-blue-600 font-medium mb-3">{lesson.category}</p>
                                ...
                                {/* Difficulty Indicator */}
                                    ...
                                    <div className="flex space-x-1">
                                        {[1, 2, 3].map((dot) => (
                                            <div
                                                key={dot}
                                                className={`w-2 h-2 rounded-full ${dot <= (lesson.difficulty === 'beginner' ? 1 : lesson.difficulty === 'intermediate' ? 2 : 3)
