...
import { Mic, Clock, Star, Users, ChevronRight, Pen, Award, RotateCcw } from 'lucide-react';
...
const Speaking = () => {
  ...
  useEffect(() => {
    const fetchLessons = async () => {
      ...
      try {
        ...
        const response = await getSkillLessonsByType('speaking', pagination.limit, pagination.offset);
        console.log('Lessons response:', response);
        if (response.status === 'success') {
          const speakingLessons = response.data || [];

          // Lấy thông tin tiến độ cho từng bài học
          const lessonsWithProgress = await Promise.all(
            speakingLessons.map(async (lesson) => {
              try {
                const progressResponse = await getLessonProgressAndAnswers(lesson.skill_id, 'speaking');
          ...
          setPagination(prev => ({ ...prev, total: response.pagination?.total || speakingLessons.length }));
        } else {
  ...
  const handleLessonAction = (lesson, action) => {
    if (action === 'review') {
      ...
      navigate(`/speaking/${lesson.skill_id}?mode=review`);
    } else if (action === 'retry') {
      // Làm lại bài học
      navigate(`/speaking/${lesson.skill_id}?mode=retry`);
    } else {
      // Vào bài học bình thường (start hoặc continue)
      navigate(`/speaking/${lesson.skill_id}`);
    }
  };
      ...
      <div className="bg-gradient-to-r from-teal-600 to-cyan-600 text-white">
        <div className="max-w-7xl mx-auto px-4 py-16">
          <div className="text-center">
            ...
            <h1 className="text-4xl font-bold mb-4">Speaking Skills</h1>
            <p className="text-xl text-teal-100 max-w-2xl mx-auto">
              Nâng cao kỹ năng nói tiếng Anh của bạn với các cuộc hội thoại tương tác và luyện phát âm.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-teal-500">
            <div className="flex items-center">
              <Mic className="w-8 h-8 text-teal-600 mr-3" />
              <div>
