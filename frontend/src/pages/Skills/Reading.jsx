...
import { BookOpen, Clock, Star, Users, ChevronRight, Pen, Award, RotateCcw } from 'lucide-react';
...
const Reading = () => {
  ...
  useEffect(() => {
    const fetchLessons = async () => {
      ...
      try {
        ...
        const response = await getSkillLessonsByType('reading', pagination.limit, pagination.offset);
        console.log('Lessons response:', response);
        if (response.status === 'success') {
          const readingLessons = response.data || [];

          // Lấy thông tin tiến độ cho từng bài học
          const lessonsWithProgress = await Promise.all(
            readingLessons.map(async (lesson) => {
              try {
                const progressResponse = await getLessonProgressAndAnswers(lesson.skill_id, 'reading');
          ...
          setPagination(prev => ({ ...prev, total: response.pagination?.total || readingLessons.length }));
        } else {
  ...
  const handleLessonAction = (lesson, action) => {
    if (action === 'review') {
      ...
      navigate(`/reading/${lesson.skill_id}?mode=review`);
    } else if (action === 'retry') {
      // Làm lại bài học
      navigate(`/reading/${lesson.skill_id}?mode=retry`);
    } else {
      // Vào bài học bình thường (start hoặc continue)
      navigate(`/reading/${lesson.skill_id}`);
    }
  };
      ...
      <div className="bg-gradient-to-r from-orange-600 to-red-600 text-white">
        <div className="max-w-7xl mx-auto px-4 py-16">
          <div className="text-center">
            ...
            <h1 className="text-4xl font-bold mb-4">Reading Skills</h1>
            <p className="text-xl text-orange-100 max-w-2xl mx-auto">
              Nâng cao khả năng đọc hiểu tiếng Anh của bạn với các đoạn văn hấp dẫn và bài tập tương tác.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-orange-500">
            <div className="flex items-center">
              ...
              <div>
                <p className="text-sm text-gray-600">Tỏng bài học</p>
