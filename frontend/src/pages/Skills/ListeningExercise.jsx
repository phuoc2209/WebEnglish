...
const ListeningExercise = () => {
        ...
        const fetchLessonAndProgress = async () => {
            ...
            try {
                ...
                const [lessonResponse, progressResponse] = await Promise.all([
                    ...
                    getLessonProgressAndAnswers(skillId, 'listening')
                ]);
                ...
                if (lessonResponse.status === 'success') {
                    ...
                    console.log('Audios:', lessonResponse.data.audios);
                    console.log('Videos:', lessonResponse.data.videos);
                    console.log('Exercises:', lessonResponse.data.exercises);
                } else {
    ...
    // Helper functions to get media and transcript
    const getAudioUrl = () => {
        if (lesson?.audios && lesson.audios.length > 0) {
            return lesson.audios[0].audio_url;
        }
        return null;
    };
    ...
    const getVideoUrl = () => {
        if (lesson?.videos && lesson.videos.length > 0) {
            return lesson.videos[0].video_url;
        }
        return null;
    };

    const getTranscript = () => {
        // Check audio transcript first
        if (lesson?.audios && lesson.audios.length > 0 && lesson.audios[0].transcript) {
            return lesson.audios[0].transcript;
        }
        // Check video transcript
        if (lesson?.videos && lesson.videos.length > 0 && lesson.videos[0].transcript) {
            return lesson.videos[0].transcript;
        }
        return null;
    };
    ...
    const getQuizQuestions = () => {
        ...
        return lesson.exercises.filter(exercise => exercise.question && exercise.options);
    };
    ...
    const handleSeek = (e) => {
        ...
        const width = rect.width;
            const seekTime = (clickX / width) * media.duration;
                    ...
                    <div className="space-y-6">
                        <div className="bg-white rounded-xl shadow-lg p-6">
                            <div className="flex items-center mb-4">
                                ...
                                <h2 className="text-xl font-semibold">Listening Material</h2>
                            </div>
                    ...
                    <div className="space-y-6">
                        <div className="bg-white rounded-xl shadow-lg p-6">
                            <div className="flex items-center justify-between mb-6">
                                <h2 className="text-xl font-semibold">Listening Comprehension Quiz</h2>
                            ...
                            {!showResults ? (
                                <>
                                    {getQuizQuestions().length > 0 ? (
                                        <div className="space-y-6">
                                            {getQuizQuestions().map((exercise, index) => (
                                                <div
                                ...
                                <div className="text-center space-y-6">
                                    ...
                                    <p className="text-gray-600">
                                        You got {Math.round((score / 100) * getQuizQuestions().length)} out of {getQuizQuestions().length} questions correct.
                                    </p>
                        ...
                        {/* Listening Tips */}
                        <div className="bg-white rounded-xl shadow-lg p-6">
                            <h3 className="text-lg font-semibold mb-4">Listening Tips</h3>
                            <div className="space-y-3 text-sm text-gray-600">
                                <div className="flex items-start">
                                    <div className="w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3 flex-shrink-0"></div>
                                    <p>Listen carefully to the audio/video before answering questions</p>
                                </div>
                                <div className="flex items-start">
                                    <div className="w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3 flex-shrink-0"></div>
                                    <p>Pay attention to key words and phrases</p>
                                </div>
                                <div className="flex items-start">
                                    <div className="w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3 flex-shrink-0"></div>
                                    <p>You can replay the audio/video as many times as needed</p>
                                </div>
                            </div>
                        </div>
                    </div>
